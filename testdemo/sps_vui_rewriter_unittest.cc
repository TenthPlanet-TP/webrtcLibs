...
// #define EXPECT_EQ(a, b)     do { assert((a) == (b) && "expert eq"); } while (false)
// #define EXPECT_FALSE(a, b)  do { assert((a) != (b) && "expert false"); } while (false)
...
//   ASSERT_TRUE(sps);
  printf("ASSERT_TRUE: %d\n", sps.has_value());
//   printf("result: %d\n", result);
//   printf("expected_parse_result: %d\n", expected_parse_result);
//   printf("ASSERT_TRUE: %d\n", expected_parse_result == result);
...
void TestSps2(const VuiHeader& vui,
             const ColorSpace* color_space,
             SpsVuiRewriter::ParseResult expected_parse_result) {
//   rtc::LogMessage::LogToDebug(rtc::LS_VERBOSE);
//   rtc::Buffer original_sps;
//   GenerateFakeSps(vui, &original_sps);
	...
	// 只能传入 rbsp, 所以要去掉 start code 和 nalu type
	const uint8_t original_sps[] = { /*0x00, 0x00, 0x00, 0x01, 0x67,*/ 0x64, 0x00, 0x28, 0xac, 0x34, 0xec, 0x05, 0x00, 0x5b, 0x90
								/*0x00, 0x00, 0x00, 0x01, 0x68, 0xea, 0x43, 0x8b*/ };

  absl::optional<SpsParser::SpsState> sps;
  rtc::Buffer rewritten_sps;
  SpsVuiRewriter::ParseResult result = SpsVuiRewriter::ParseAndRewriteSps(
      original_sps, arraysize(original_sps), &sps, color_space,
      &rewritten_sps, SpsVuiRewriter::Direction::kIncoming);
  EXPECT_EQ(expected_parse_result, result);
//   ASSERT_TRUE(sps);
  printf("ASSERT_TRUE: %d\n", sps.has_value());
...
//   printf("ASSERT_TRUE: %d\n", expected_parse_result == result);
  ...
  if (vui.vui_parameters_present_flag) {
	...
	printf("----------- input start -------------\n");
	for (int i = 0; i < arraysize(original_sps); i++) {
		printf("0x%02x, ", original_sps[i]);
	}
	printf("\n----------- input end -------------\n\n");

	printf("----------- output start -------------\n");
	if (rewritten_sps.data() != nullptr && rewritten_sps.size() > 0) {
		for (int i = 0; i < rewritten_sps.size(); i++) {
			printf("0x%02x, ", rewritten_sps.data()[i]);
		}
	}
	printf("\n----------- output end -------------\n\n");
...
int main(void)
{
	const ColorSpace* color_space = &kColorSpaceH264Default;

	SpsVuiRewriter::ParseResult expected_parse_result = SpsVuiRewriter::ParseResult::kVuiRewritten;

	TestSps(kVuiNotPresent, color_space, expected_parse_result);
	printf("--------\n");
	TestSps(kVuiNoBitstreamRestriction, color_space, expected_parse_result);
	printf("--------\n");
	TestSps(kVuiNoFrameBuffering, color_space, expected_parse_result);
	printf("--------\n");
	TestSps(kVuiFrameBuffering, color_space, expected_parse_result);
	printf("--------\n");
	TestSps(kVuiNoVideoSignalType, color_space, expected_parse_result);
	printf("--------\n");
	TestSps(kVuiLimitedRangeNoColourDescription, color_space, expected_parse_result);
	printf("--------\n");
	TestSps(kVuiFullRangeNoColourDescription, color_space, expected_parse_result);
	printf("--------\n");
	TestSps(kVuiLimitedRangeBt709Color, color_space, expected_parse_result);
	printf("--------\n");

	printf("---zfw start-----\n");
	TestSps2(kVuiNoFrameBuffering, color_space, expected_parse_result);
	printf("---zfw end-----\n");

	{
		...
		printf("----------- output start -------------\n");
		rtc::Buffer output_buffer = SpsVuiRewriter::ParseOutgoingBitstreamAndRewrite(buffer, nullptr);
		if (output_buffer.data() != nullptr && output_buffer.size() > 0) {
...

	{
		...
		// 		0x00, 0x00, 0x00, 0x01, 0x67, 0x64, 0x00, 0x28, 0xac, 0x34, 0xec, 0x05, 0x00, 0x5b, 0x90,
		// 		0x00, 0x00, 0x00, 0x01, 0x68, 0xea, 0x43, 0x8b, 
		// 		0x00, 0x00, 0x00, 0x01, 0x65, 0x88, 0x80, 0x20, 0x00 };
		...
		// rtc::Buffer buffer(original_buffer);
		// buffer.AppendData(kStartSequence);

		printf("----------- input start -------------\n");
		for (int i = 0; i < arraysize(original_buffer); i++) {
			printf("0x%02x, ", original_buffer[i]);
		}
		printf("\n----------- input end -------------\n\n");

		printf("----------- output start -------------\n");
		// rtc::Buffer output_buffer = SpsVuiRewriter::ParseOutgoingBitstreamAndRewrite(buffer, nullptr);
		rtc::Buffer output_buffer = SpsVuiRewriter::ParseOutgoingBitstreamAndRewrite(original_buffer, nullptr);
		if (output_buffer.data() != nullptr && output_buffer.size() > 0) {
			for (int i = 0; i < output_buffer.size(); i++) {
				printf("0x%02x, ", output_buffer.data()[i]);
			}
		}
		printf("\n----------- output end -------------\n\n");
		output_buffer.Clear();
