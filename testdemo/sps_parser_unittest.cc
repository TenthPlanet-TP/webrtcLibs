...
// #define EXPECT_FALSE(a, b)  assert((a) != (b))
...
{
	{
		...
		H264SpsParserTest test;
		test.sps_ = SpsParser::ParseSps(buffer, arraysize(buffer));
		printf("EXPECT_TRUE: %d\n", test.sps_.has_value());
		EXPECT_EQ(1280u, test.sps_->width);
		EXPECT_EQ(720u, test.sps_->height);

		printf("pic_order_cnt_type: %d\n", test.sps_->pic_order_cnt_type);
		printf("max_num_ref_frames: %d\n", test.sps_->max_num_ref_frames);
		printf("vui_params_present: %d\n", test.sps_->vui_params_present);
	}

	{
		...
		test.sps_ = SpsParser::ParseSps(buffer, arraysize(buffer));
		printf("EXPECT_TRUE: %d\n", test.sps_.has_value());
		EXPECT_EQ(640u, test.sps_->width);
		EXPECT_EQ(360u, test.sps_->height);
...

	{
		...
		test.sps_ = SpsParser::ParseSps(buffer, arraysize(buffer));
		printf("EXPECT_TRUE: %d\n", test.sps_.has_value());
		EXPECT_EQ(200u, test.sps_->width);
		EXPECT_EQ(400u, test.sps_->height);
...

	{
		...
		const uint8_t buffer[] = { /*0x00, 0x00, 0x00, 0x01, 0x67,*/ 0x64, 0x00, 0x28, 0xac, 0x34, 0xec, 0x05, 0x00, 0x5b, 0x90
								   /*0x00, 0x00, 0x00, 0x01, 0x68, 0xea, 0x43, 0x8b*/ };
		// EXPECT_TRUE(
		// 	static_cast<bool>(sps_ = SpsParser::ParseSps(buffer, arraysize(buffer))));
		H264SpsParserTest test;
		test.sps_ = SpsParser::ParseSps(buffer, arraysize(buffer));		// 只能传入 rbsp
		printf("EXPECT_TRUE: %d\n", test.sps_.has_value());
		EXPECT_EQ(1280u, test.sps_->width);
		EXPECT_EQ(720u, test.sps_->height);
...

	{
		...
		H264SpsParserTest test;
		test.sps_ = SpsParser::ParseSps(buffer.data(), buffer.size());
		printf("EXPECT_TRUE: %d\n", test.sps_.has_value());

		EXPECT_EQ(320u, test.sps_->width);
		EXPECT_EQ(180u, test.sps_->height);
		EXPECT_EQ(1u, test.sps_->id);
...

	{
		...
		EXPECT_EQ(156u, test.sps_->width);
		EXPECT_EQ(122u, test.sps_->height);
		EXPECT_EQ(2u, test.sps_->id);
...

	{
		...
		test.sps_ = SpsParser::ParseSps(buffer, arraysize(buffer));
		printf("EXPECT_TRUE: %d\n", test.sps_.has_value());
		EXPECT_EQ(1920u, test.sps_->width);
		EXPECT_EQ(1080u, test.sps_->height);
...

	{
		...
		EXPECT_EQ(180u, test.sps_->height);
		EXPECT_EQ(1u, test.sps_->id);
		EXPECT_EQ(4u, test.sps_->log2_max_frame_num);
		...
		EXPECT_EQ(180u, test.sps_->height);
		EXPECT_EQ(1u, test.sps_->id);
		EXPECT_EQ(32u, test.sps_->log2_max_frame_num);
...

	{
		...
		EXPECT_EQ(180u, test.sps_->height);
		EXPECT_EQ(1u, test.sps_->id);
		EXPECT_EQ(4u, test.sps_->log2_max_pic_order_cnt_lsb);
		...
		EXPECT_EQ(1u, test.sps_->id);
		printf("----here start--\n");
		EXPECT_EQ(32u, test.sps_->log2_max_pic_order_cnt_lsb);
		printf("----here end----\n");
